...
const TargetSelector: React.FC<TargetSelectorProps> = ({
  ...
  return <div className="space-y-8 animate-fade-in p-4 py-[18px]">
      <div className="px-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
        {targets.map(target => <div key={target.id} onClick={() => handleTargetSelect(target.id)} className="">
            <div className="flex flex-col h-full justify-center items-center text-center gap-3 py-0 my-[10px]">
      ...
      {showCustom && <div className="animate-slide-up space-y-4 px-2">
          <div className="modern-form flex flex-col md:flex-row gap-3">
            <input type="text" value={customTarget} onChange={e => setCustomTarget(e.target.value)} placeholder="Enter what/who is bothering you..." className="flex-1 px-4 py-3 rounded-md border" />
            <button onClick={handleCustomTargetSubmit} disabled={!customTarget.trim()} className="bg-primary hover:bg-primary/90 text-primary-foreground px-6 py-3 rounded-md transition-colors disabled:opacity-50">
              Continue
            </button>
          </div>
